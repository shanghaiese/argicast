(function ($) {
    (function (window, undefined) {
        ...
        //to transfer cookies values to moss domain.
        //        function ais_cookieval(cname, cvalue) {
	...
	((secure == true) ? "; secure" : "");
        ...
        function HideControlsForWidget() {
            ...
            $('#' + AgriCast.idPrefix + 'cngStat').addClass('hide'); //Hide the Stations link for Widget Request
        ...
        AgriCast.Pub = config.pub;
            ...
            if (config.overrideCookie == 'true' || getCookie(AgriCast.clientID) == null) {
                ...
                if (AgriCast.Culture != config.culture) {
                    var cookievalue = readcookievalues(AgriCast.clientID);
                    AIS_WriteCookie(AgriCast.clientID, cookievalue.ServiceName + '|' + cookievalue.Module + '|' + config.culture + '|' +
                                                                cookievalue.CountryCode + '|' + cookievalue.placeName + '|' + cookievalue.latitude + '|' + cookievalue.longitude + '|' + cookievalue.OverRideCookie + '|' + cookievalue.Widget + '|' + cookievalue.Unit);
                }
        ...
        }; //end of AgriCast,reporterror

        AgriCast.LoadSearch = function (divid, culture, country, place) {

            AgriCast.DivID = divid;
            if (getCookie(divid) != null) {
                getcookievalues(divid);
            ...
            $.ajax({
                ...
                data: "pub=" + AgriCast.Pub + "&method=LocationSearch&culture=" + AgriCast.Culture + "&country=" + AgriCast.SelectedCountry + "&place=" +
            escape(AgriCast.Place) + "&iswidget=" + AgriCast.isWidget
            + "&clientID=" + AgriCast.clientID + "&module=" + AgriCast.Module + "&lat=" + AgriCast.Lat + "&lng=" + AgriCast.Lng + "&unit=" + AgriCast.unit + "&overrideCookie=" + AgriCast.OverrideCookie,
                success: function (data) {
                    //changes for multiple instance issue on one page -start
                    divid = data.Clientid;
                    $('#' + divid).html(data.PgHTML);
                    AgriCast.idPrefix = data.Clientid;
                    AgriCast.DivID = data.Clientid;
                    if (getCookie(data.Clientid) != null) {
                        getcookievalues(data.Clientid);
                    }
                    // AgriCast.isWidget = data.isWidget;
                    if (AgriCast.Lat != '' && AgriCast.Lng != '') {
                        $('#' + AgriCast.idPrefix + 'divLocSearch').attr('class', 'hide');

                    }
                    //changes for multiple instance issue on one page -end
                    //changes for unit implementation-start
                    if (AgriCast.unit != '') {
                        $('#' + AgriCast.idPrefix + 'ddlUnit').val(AgriCast.unit.toUpperCase());
                    }
                    $('#' + AgriCast.idPrefix + 'btnsave').unbind('click');
                    $('#' + AgriCast.idPrefix + 'btnsave').click(function () {
                        AgriCast.clientID = this.id.substring(0, this.id.indexOf("btnsave"));
                        var cookievalue = readcookievalues(AgriCast.clientID);
                        if (cookievalue.Unit.toLowerCase() != $('#' + AgriCast.clientID + 'ddlUnit :selected').val().toLowerCase()) {
                            AIS_WriteCookie(AgriCast.clientID, cookievalue.ServiceName + '|' + cookievalue.Module + '|' + cookievalue.Culture + '|' +
                                                                cookievalue.CountryCode + '|' + cookievalue.placeName + '|' + cookievalue.latitude + '|' + cookievalue.longitude + '|' + cookievalue.OverRideCookie + '|' + cookievalue.Widget + '|' + $('#' + AgriCast.clientID + 'ddlUnit :selected').val().toLowerCase());

                            getcookievalues(AgriCast.clientID);
                            AgriCast.btnSearchClick("onLoadSearch", AgriCast.clientID);
                        }
                    });
                    ...
                    if ($.trim($('#' + AgriCast.idPrefix + 'txtPlace').val()) != '') {
                        ...
                        AgriCast.btnSearchClick("onLoadSearch", AgriCast.idPrefix);
                    ...
                    $('.ag_imgSpin').hide();
                    $('#' + AgriCast.idPrefix + 'ag_stnName').hide();
            ...
            //bind autocomplete event to newly created textbox--start of function
            $('#' + divid).delegate('#' + divid + 'divLocSearch', "focus", function () {
                AgriCast.idPrefix = divid;
                AgriCast.clientID = divid;
                if (getCookie(AgriCast.idPrefix) != null) {
                    getcookievalues(AgriCast.idPrefix);
                }
                if (!AgriCast.SelectedCountry) {
                ...
                $('#' + divid + 'ddlCountry').unbind('change');
                ...
                /*This does not work in local env. uncomment this before release*/
                $('#' + AgriCast.idPrefix + 'txtPlace').autocomplete({
                    ...
                    source: function (request, response) {
                        $.ajax({
                            ...
                            cache: true,
                            url: AgriCast.HttpHandlerPath,
                            data: "pub=" + AgriCast.Pub + "&method=LocDetAuto&country=" + $('#' + AgriCast.idPrefix + 'ddlCountry' + ' option:selected').val() + "&culture=" + AgriCast.Culture + "&place=" + escape($('#' + AgriCast.idPrefix + 'txtPlace').val()) + "&clientID=" + AgriCast.clientID,
                            success: function (data) {
                                AgriCast.idPrefix = data.Clientid;
                                if (getCookie(data.Clientid) != null) {
                                    getcookievalues(data.Clientid);
                    ...
                    select: function (event, ui) {
                        AgriCast.idPrefix = this.id.substring(0, this.id.indexOf("txtPlace"));
                        ...
                        //                    var auto = {ClientId: AgriCast.idPrefix, auto: "auto"};
                        AgriCast.btnSearchClick("auto", AgriCast.idPrefix);
                        return false;
                    }
                }); //end of autocomplete            

                /*End of Comment*/
            }); //bind autocomplete event to newly created textbox--End of function

        }; //End of AgriCast.LoadSearch function

        //Show invalid lat long search error message - begin
        var ValidToLoadModules;
        //Show invalid lat long search error message - end
        AgriCast.btnSearchClick = function (obj, id) {
            //debugger;
            if (typeof (id) != 'undefined') {
                AgriCast.idPrefix = id;
                //            if (getCookie(AgriCast.idPrefix) != null) {
                //                getcookievalues(AgriCast.idPrefix);
                //            }
            }
            ...
            AgriCast.SelectedCountry = $("#" + AgriCast.idPrefix + "ddlCountry" + " option:selected").val();
            ...
            var cookievalue = readcookievalues(AgriCast.idPrefix);


            AIS_WriteCookie(AgriCast.idPrefix, cookievalue.ServiceName + '|' + cookievalue.Module + '|' + cookievalue.Culture + '|' +
                                                                AgriCast.SelectedCountry + '|' + AgriCast.Place + '|' + AgriCast.Lat + '|' + AgriCast.Lng + '|' +
                                                                 cookievalue.OverRideCookie + '|' + cookievalue.Widget + '|' +
                                                                 cookievalue.Unit);
            ...
            $.ajax({
                ...
                cache: true,
                url: AgriCast.HttpHandlerPath,
                data: "pub=" + AgriCast.Pub + "&method=LocDetStn&module=" + AgriCast.Module + "&country=" + AgriCast.SelectedCountry + "&culture=" + AgriCast.Culture + "&place=" + escape(AgriCast.Place) + "&lat=" + AgriCast.Lat + "&lng=" + AgriCast.Lng + "&iswidget=" + AgriCast.isWidget + "&clientID=" + AgriCast.clientID,
                success: function (data) {
                    AgriCast.idPrefix = data.Clientid;
                    divid = data.Clientid; //added by ashish
                    if (getCookie(data.Clientid) != null) {
                        getcookievalues(data.Clientid);
                    }
                    //Locations
                    ...
                    $('#' + AgriCast.idPrefix + 'gvLocation td').each(function (inx) {
                        $(this).click(function () {
                            AgriCast.idPrefix = this.parentNode.parentNode.parentNode.id.substring(0, this.parentNode.parentNode.parentNode.id.indexOf("gvLocation"));
                            ...
                            AgriCast.btnSearchClick("tab", AgriCast.idPrefix);
                    ...
                    if (lengthStn > 1) {
                        ...
                        for (var i = 0; i < lengthStn; i++) {
                            $('#' + AgriCast.idPrefix + 'gvStations > tbody:last').append('<tr id=' + i + '><td style="border:1px solid grey; padding-left:5px; cursor:pointer;">' + dataStn[i].Name + '</td><td style="border:1px solid grey; padding-left:5px; cursor:pointer;">' + dataStn[i].DistText + '</td></tr>');
                        ...
                        $('#' + AgriCast.idPrefix + 'gvStations td').each(function (inx) {
                            $(this).click(function () {
                                ...
                                $('#' + AgriCast.idPrefix + 'lblStnName')[0].innerHTML = dataStn[this.parentNode.id].Name;
                                $('#' + AgriCast.idPrefix + 'lblDistText')[0].innerHTML = dataStn[this.parentNode.id].DistText;
                                AgriCast.LoadModules(AgriCast.Module, AgriCast.idPrefix);
                            });
                        });
                    }
                    else if (lengthStn == 1) {
                        ...
                        $('#' + AgriCast.idPrefix + 'gvStations').hide();
                        $('#' + AgriCast.idPrefix + 'ag_stnName').hide();
                        $('#' + AgriCast.idPrefix + 'cngStat').click();
                    ...
                    if ((length > 1 && obj != "tab") && (obj.result == null || obj.result == "")) {
                        $('#' + AgriCast.idPrefix + 'divLocation').attr('class', 'show ag_divLocation');
                        ...
                        if (ValidToLoadModules == "true") {
                            ...
                            cookievalue = readcookievalues(AgriCast.idPrefix);
                            AIS_WriteCookie(AgriCast.idPrefix, cookievalue.ServiceName + '|' + cookievalue.Module + '|' + cookievalue.Culture + '|' +
                                                                cookievalue.CountryCode + '|' + cookievalue.placeName + '|' + AgriCast.Lat + '|' + AgriCast.Lng + '|' +
                                                                 cookievalue.OverRideCookie + '|' + cookievalue.Widget + '|' +
                                                                 cookievalue.Unit);

                            /*IM01173263 - New Agricast - EmbeddedJS- location search querystring -  Begin*/
                            AgriCast.LoadModules(AgriCast.Module, AgriCast.idPrefix); //Load module only if the location is found
                        } else {
        ...
        AgriCast.ChangeLocation = function (divid) {
            ...
            $('#' + AgriCast.idPrefix + 'divLocation').attr('class', 'hide');
            $('#' + AgriCast.idPrefix + 'divStations').attr('class', 'hide');
            $('#' + AgriCast.idPrefix + 'divsettings').attr('class', 'hide');
            ...
            $('#' + AgriCast.idPrefix + 'ChangeLoc a[id=' + AgriCast.idPrefix + 'cngLoc]').click(function () {
                AgriCast.idPrefix = this.parentNode.id.substring(0, this.parentNode.id.indexOf("ChangeLoc"));
                ...
                if ($('#' + AgriCast.idPrefix + 'close').length == 0) {
                    $('<div id="' + AgriCast.idPrefix + 'close" class="ag_close" ></div>"').appendTo($('#' + AgriCast.idPrefix + 'ChangeLoc'));
                }
                $('#' + AgriCast.idPrefix + 'close').click(function () {
                    AgriCast.idPrefix = this.id.substring(0, this.id.indexOf("close"));
                    $('#' + AgriCast.idPrefix + 'divLocSearch').attr('class', 'hide');
                    $('#' + AgriCast.idPrefix + 'divLocation').attr('class', 'hide');
                    $('#' + AgriCast.idPrefix + 'divStations').attr('class', 'hide');
                    $('#' + AgriCast.idPrefix + 'divsettings').attr('class', 'hide');
                    ...
                    $('#' + AgriCast.idPrefix + 'divStationDetails').attr('class', 'show StationDetails ag_divStationDetails');
                    /*IM01175094 - New Agricast - Embedded JS - showing/hidding the locationinformation - END*/
                    //end of change
                    $(this).remove();
                    return false;
            ...
            $('#' + AgriCast.idPrefix + 'ChangeLoc a[id=' + AgriCast.idPrefix + 'cngStat]').click(function () {
                AgriCast.idPrefix = this.parentNode.id.substring(0, this.parentNode.id.indexOf("ChangeLoc"));
                $('#' + AgriCast.idPrefix + 'divLocSearch').attr('class', 'hide');
                $('#' + AgriCast.idPrefix + 'divLocation').attr('class', 'hide');
                $('#' + AgriCast.idPrefix + 'divsettings').attr('class', 'hide');
                $('#' + AgriCast.idPrefix + 'divStations').attr('class', 'show ag_divStations');
            ...
            //changes for unit implementation-start
            $('#' + AgriCast.idPrefix + 'ChangeLoc a[id=' + AgriCast.idPrefix + 'imgSettings]').click(function () {
                AgriCast.idPrefix = this.parentNode.id.substring(0, this.parentNode.id.indexOf("ChangeLoc"));
                $('#' + AgriCast.idPrefix + 'divLocSearch').attr('class', 'hide');
                $('#' + AgriCast.idPrefix + 'divLocation').attr('class', 'hide');
                $('#' + AgriCast.idPrefix + 'divStations').attr('class', 'hide');
                if (AgriCast.isWidget == 'true') {
                    $('#' + AgriCast.idPrefix + 'divsettings').attr('class', 'widgetContainer'); //Width of Location Search div
                }
                else {
                    $('#' + AgriCast.idPrefix + 'divsettings').attr('class', 'container');
                }


                if ($('#' + AgriCast.idPrefix + 'close').length == 0) {
                    $('<div id="' + AgriCast.idPrefix + 'close" class="ag_close" ><img src="' + closeImg + '" alt="Close"></img></div>"').appendTo($('#' + AgriCast.idPrefix + 'ChangeLoc'));
                }
                $('#' + AgriCast.idPrefix + 'ChangeLoc img').click(function () {
                    AgriCast.idPrefix = this.parentNode.id.substring(0, this.parentNode.id.indexOf("close"));
                    $('#' + AgriCast.idPrefix + 'divLocSearch').attr('class', 'hide');
                    $('#' + AgriCast.idPrefix + 'divLocation').attr('class', 'hide');
                    $('#' + AgriCast.idPrefix + 'divStations').attr('class', 'hide');
                    $('#' + AgriCast.idPrefix + 'divsettings').attr('class', 'hide');
            ...
            //changes for unit implementation-end
        };
        ...
        AgriCast.LoadModules = function (mod, divid) {
            ...
            if (getCookie(divid) != null) {
                ...
                getcookievalues(divid);
                try {
                    ais_cookieval(divid, getCookie(divid).toString());
                }
                catch (err) {
                }
            ...
            + "&stnname=" + AgriCast.StationName + "&iswidget=" + AgriCast.isWidget + "&clientID=" + AgriCast.clientID + "&unit=" + AgriCast.unit,
                success: function (data) {
                    if (getCookie(data.Clientid) != null) {
                        AgriCast.idPrefix = data.Clientid;
                        getcookievalues(data.Clientid);
                    ...
                    if (typeof data != "undefined") //this occurs when the first time load if the location does not belong to the country selected
                    {
                        ...
                        if (AgriCast.isWidget != "true") {
                            ...
                            var strStnDetail = "";
                            $('#' + AgriCast.idPrefix + 'ag_stnName span').each(function () {
                                if (strStnDetail == "")
