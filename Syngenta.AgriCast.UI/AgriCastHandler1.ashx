...
public class AgriCastHandler : IHttpHandler, IRequiresSessionState
{
   ...
   // bool blOverrideCookie = false;
    ...
    
    {
        try
        {
           ...
           // blOverrideCookie = bool.Parse(context.Request.QueryString["overrideCookie"] ?? DEFAULT_OVERRIDECOOKIE.ToString());
            ...
            appurl.Append(context.Request.Url.LocalPath.Split(new string[] { "AgriCastHandler1.ashx" }, StringSplitOptions.None)[0].ToString());
            ...
            //objDataPointInfo.stationName = values[8];
    ...
    protected string GetLocationSearch(string culture, string pub)
    {
        ...
        cngloc.ID = CONTROLS_ID_PREFIX + "cngLoc";
        cngloc.HRef = "#";
        cngloc.InnerText = locSvc.getTranslatedText("ChangeLocation", objSvcInfo.Culture);
        cngloc.Attributes.Add("class", "cngLoc");
        
        HtmlAnchor cngstn = new HtmlAnchor();
        cngstn.ID = CONTROLS_ID_PREFIX + "cngStat";
        cngstn.HRef = "#";
        cngstn.InnerHtml = locSvc.getTranslatedText("ResNearbyStations", objSvcInfo.Culture);
        cngstn.Attributes.Add("class", "cngStat");
        
        //changes for unit implementation-start
        HtmlAnchor imgSettings = new HtmlAnchor();
        imgSettings.ID = CONTROLS_ID_PREFIX + "imgSettings";
        imgSettings.HRef = "#";
        imgSettings.InnerHtml = locSvc.getTranslatedText("Configure", objSvcInfo.Culture);
        imgSettings.Attributes.Add("class", "imgSettings");
        ...
        //changes for unit implementation-start
        chgLoc.Controls.Add(new LiteralControl("&nbsp;&nbsp;&nbsp;"));
        chgLoc.Controls.Add(imgSettings);
        //changes for unit implementation-end
        Panel p = new Panel();
        ...
        imgSearch.Attributes.Add("class", "ag_btnSearch");
        
        Panel pnlStnName = new Panel();
        pnlStnName.ID = CONTROLS_ID_PREFIX + "ag_stnName";
        ...
        pnlSettings.ID = CONTROLS_ID_PREFIX + "divsettings";
        pnlSettings.CssClass = CSS_CLASS_HIDE;
        Label lblUnit = new Label();
        lblUnit.ID = CONTROLS_ID_PREFIX + "lblUnit";
        lblUnit.CssClass = "lblUnit";
        lblUnit.Text = objSvcPre.getTranslatedText("unit", objSvcInfo.Culture);
        lblUnit.Visible = true;

        DropDownList ddlUnit = new DropDownList();
        ddlUnit.ID = CONTROLS_ID_PREFIX + "ddlUnit";
        ddlUnit.CssClass = "ddlUnit";
        DataTable dtUnit = new DataTable();
        dtUnit.Columns.Add("Value");
        dtUnit.Columns.Add("Code");
        DataRow drUnit = dtUnit.NewRow();
        drUnit["Value"] = locSvc.getTranslatedText("metric", objSvcInfo.Culture);
        drUnit["Code"] = "METRIC";
        dtUnit.Rows.Add(drUnit);
        drUnit = dtUnit.NewRow();
        drUnit["Value"] = locSvc.getTranslatedText("imperial", objSvcInfo.Culture);
        drUnit["Code"] = "IMPERIAL";
        dtUnit.Rows.Add(drUnit);
        ...
        HtmlInputButton btnsave = new HtmlInputButton();
        btnsave.Attributes.Add("type", "button");   
        btnsave.ID = CONTROLS_ID_PREFIX + "btnsave";
        btnsave.Value = locSvc.getTranslatedText("Save", objSvcInfo.Culture);
        btnsave.Attributes.Add("class", "btnsave");
        pnlSettings.Controls.Add(lblUnit);
        pnlSettings.Controls.Add(new LiteralControl(":"));
       
        pnlSettings.Controls.Add(ddlUnit);
        ...
        pModMaster.CssClass = "ag_divModMaster";
        ...
        sbJson.Append(@""",""Clientid"":""");
        sbJson.Append(CONTROLS_ID_PREFIX);
        sbJson.Append(@""" }");
    ...
    public string GetModules(string PubName, string module)
    {
        ...
        StringBuilder sbJson = new StringBuilder();
        /*INCIDENT IM02362401: mySyngenta "External" tab: Weather is not working as expected - see attached - BEGIN */
        sbJson.Append(@"{""ModHTML"":""");
        /*INCIDENT IM02362401: mySyngenta "External" tab: Weather is not working as expected - see attached - END */
        //New Widget Request
        if (isWidget)
        {
    ...
    public string GetHtmlForWidget(DataTable dtInput)
    {
        ...
        sbHTML.Append("<div id=\"'" + CONTROLS_ID_PREFIX + "'divModules\"><div>");
        sbHTML.Append("<div id=\"'" + CONTROLS_ID_PREFIX + "'tblwidget\" class=\"widgetTblwidget\">");
    ...
    public string ConvertDataSetToJson(DataSet ds)
    {
        ...
        resultMain.Add("Clientid", CONTROLS_ID_PREFIX);
        JavaScriptSerializer objSer = new JavaScriptSerializer();
