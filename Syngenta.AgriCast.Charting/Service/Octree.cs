...
{
    public class OctreeQuantizer:ColorQuantizer  
    {
        /// <summary>
		...
		public OctreeQuantizer ( int maxColors, Color[] cpixels ) {
			int maxColorBits = 8;
			if ( maxColors > 255 )
		...
		public override void init() {
			for (int x = 0; x < pixels.Length; x++) _octree.AddColor ( pixels[x] ) ;
			palette = _octree.Palletize(_maxColors - 1);
		}
		private ArrayList palette;
		/// <summary>
		/// get the determined colors (does not apply to this algorithm)
		/// </summary>
		/// <returns></returns>
		...
		/// <summary>
		/// lookup the palette index of any color of the original image
		/// </summary>
		/// <param name="pixel">The pixel to quantize</param>
		/// <returns>The quantized value</returns>
		public override int lookup ( Color pixel ) {
			// Get the palette index if this non-transparent
			if ( pixel.A > 0 ) return _octree.GetPaletteIndex ( pixel ) ;
			return _maxColors;// The color at [_maxColors] is set to transparent
		}
		/// <summary>
		/// return the color at palette position x
		/// </summary>
		/// <param name="ix"></param>
		/// <returns></returns>
		public override Color getColor(int ix) {
			// First off convert the octree to _maxColors colors
			if (ix >= palette.Count) return Color.FromArgb ( 0 , 0 , 0 , 0 ) ;
			return (Color)palette[ix];
		}
		
		/// <summary>
		/// stores the original pixels
		/// </summary>
