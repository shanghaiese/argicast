...
{
    public class RS_Harvest_SugarBeet_Medium
    {
        ...
        public static DataTable RulesetSeriesList
        {
            get
            {
                ...
                RulesetSerieList.Rows.Add("MoistureSoilSilt030cm");
        ...
        private const string RS_Harvest_Medium_SugarBeet_ResLegTooWetSoilPrecip48 = "M";
        private const string RS_Harvest_Medium_SugarBeet_ResLegRain24 = "R";
        private const string RS_Harvest_Medium_SugarBeet_ResLegTooColdSoil = "C";
        private const string RS_Harvest_Medium_SugarBeet_ResLegTooColdSoil48 = "C";
        private const string RS_Harvest_Medium_SugarBeet_Default = "X";

        private static string PRECIP = "PrecipQuantity_hSum";
        private static string SOILTEMP = "soiltemp";
        private static string MOISTURESOIL_SILT = "MoistureSoilSilt030cm";
        ...
        static public DataTable CalculateRuleSets(IRuleSets objIRuleSets)
        {
            ...
            MOISTURESOIL_SILT = getColumnName(dtInput.Columns, "MoistureSoilSilt030cm");
        ...
        static private void CalcAllRules(int startIndex)
        {
            ...
            double fCurrHour_SoilSilt = double.Parse(dtInput.Rows[startIndex][MOISTURESOIL_SILT].ToString());
            double fCurrHour_SoilTemp = double.TryParse(dtInput.Rows[startIndex][SOILTEMP].ToString(), out i) ? i : 0;


            //check for Condition
            //<condition start="-48" end="0" decimaterule="sum" column="precip" lower="10" />
            ...
            temp = from dt in dtInputRaw.AsEnumerable()
                   where DateTime.Parse(dt[DATE].ToString()) >= dateLocal.AddHours(-24) &&
                   DateTime.Parse(dt[DATE].ToString()) <= dateLocal
                   select dt;

            //<condition start="24" end="0" decimaterule="sum" column="precip" lower="1" upper="10" />
            ...
            var temp3 = from dt in dtInputRaw.AsEnumerable()
                        where DateTime.Parse(dt[DATE].ToString()) >= dateLocal.AddHours(-48) &&
                    DateTime.Parse(dt[DATE].ToString()) < dateLocal
                        select dt;
            ...
            float fSum_FM24_T0_Precip = float.Parse(result_FM24_T0.ToList()[0].dtSumPrecip.ToString());//Temparature:-(hour -24 to 0)
            float fMin_FM48_T1_SoilTemp = float.Parse(result_FM48_T1.ToList()[0].dtMinSoilTemp.ToString());//Temparature:-(hour -48 to -1)


            //<rule value="0" letter="M" adddescription="ResLegTooWetSoilPrecip48">
            //      <!-- soil moisture too high - no planting-->
            //                <condition decimaterule="avg" column="MoistureSoilSilt030cm" interpolaterule="dailyto24" lower="99" />
            //                <condition start="-48" end="0" decimaterule="sum" column="precip" lower="10" />
            //            </rule>

            if (fCurrHour_SoilSilt > 99 || fSum_H48_T0_Precip > 10.0)
            {
            ...
            //    <!-- soil moisture too high - no planting-->
            //    <condition decimaterule="avg" column="MoistureSoilSilt030cm" interpolaterule="dailyto24" lower="99" />
            //    <condition start="-48" end="0" decimaterule="sum" column="precip" lower="1" upper="10" />
            //</rule>
            if (fCurrHour_SoilSilt > 99 ||
                (fSum_H48_T0_Precip > 1 && fSum_H48_T0_Precip <= 10))
            {
            ...
            //    <!--precip expected - cannot plant -->
            //    <condition start="-24" end="0" decimaterule="sum" column="precip" lower="5" upper="10" />
            //</rule>
            if (fSum_FM24_T0_Precip > 5 && fSum_FM24_T0_Precip <= 10)
            {
                if (!CheckRestrictioninList(strResLegRain24))
                {
        ...
        static private void GetOutputData(int index)
        {
            ...
            //Assign default color code if no restrcition
            if (string.IsNullOrEmpty(strCellValue))
            {
                strFinalCellValue = strDefaultCellValue;
            }
            if (blIsNight)
            {
            ...
            if (strRestList != null)
            {
                ...
                {
                    drOutput["value"] = RS_Harvest_Medium_SugarBeet_Default;
                }
                else
                {
        ...
        static private string GetRestrictionLetter(string strRest)
        {
            ...
            {
                strLetter = RS_Harvest_Medium_SugarBeet_ResLegRain24;
            }
            else if (strRest.Equals(strResLegTooColdSoil))
            {
                strLetter = RS_Harvest_Medium_SugarBeet_ResLegTooColdSoil;
            }
            else if (strRest.Equals(strResLegTooColdSoil48))
            {
                strLetter = RS_Harvest_Medium_SugarBeet_ResLegTooColdSoil48;
            }

            else if (strRest.Equals(strResLegTooWetSoilPrecip48))
            {
                strLetter = RS_Harvest_Medium_SugarBeet_ResLegTooWetSoilPrecip48;
            }
