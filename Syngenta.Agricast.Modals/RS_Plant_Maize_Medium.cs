...
{
    public class RS_Plant_Maize_Medium
    {
        ...
        private const string RS_Plant_Maize_Medium_ResLegTooColdSoil = "A";
        private const string RS_Plant_Maize_Medium_ResLegTooWetGround = "M";
        private const string RS_Plant_Maize_Medium_ResLegRain = "R";
        private const string RS_Plant_Maize_Medium_Default = "X";
        ...
        static private void CalcAllRules(int startIndex)
        {
            ...
            /*<rule value="0" letter="R" adddescription="ResLegRain" hourfrom="0" hourto="6">
                <!--precip now - cannot plant -->
                <condition decimaterule="sum" column="precip" lower="0"/>*/
            var resHours_6 = from dt in dtInputRaw.AsEnumerable()
                             where DateTime.Parse(dt[DATE].ToString()) < dateLocal.AddHours(6) &&
                             DateTime.Parse(dt[DATE].ToString()) >= dateLocal
            ...
            float fSum_H6 = float.Parse(resHours_6.ToList()[0].dtSumPrecip.ToString());//Fetch the sum of data (hour 0 to 6)
            ...
            var resHours_24 = from dt in dtInputRaw.AsEnumerable()
                              ...
                              select new
                              {
                                  //point to be noted
                                  dtSumPrecip = grp.Sum(t => double.TryParse(t[PRECIP].ToString(), out i) ? i : 0.0),
                                  dtAvgMoistureSoilClay = grp.Average(t => double.TryParse(t[MOISTURESOIL_SILT].ToString(), out i) ? i : 0.0),
            ...
            float fAvg_H24_MOISTURESOIL_SILT = float.Parse(resHours_24.ToList()[0].dtAvgMoistureSoilClay.ToString());//Fetch the Avg of data (hour 0 to 24) 
            ...
            //    <condition decimaterule="avg" column="MoistureSoilClay010cm" lower="40"/>
            if (fAvg_H24_MOISTURESOIL_SILT > 50.0)
            {
        ...
        static private void GetOutputData(int index)
        {
            ...
            if (strRestList != null)
            {
                ...
                {
                    drOutput["value"] = RS_Plant_Maize_Medium_Default;
                }
                else
                {
        ...
        static private string GetRestrictionLetter(string strRest)
        {
            ...
            {
                strLetter = RS_Plant_Maize_Medium_ResLegTooColdSoil;
            }
            else if (strRest.Equals(strResLegRain))
            {
                strLetter = RS_Plant_Maize_Medium_ResLegRain;
            }
            else if (strRest.Equals(strResLegTooWetGround))
            {
                strLetter = RS_Plant_Maize_Medium_ResLegTooWetGround;
            }
        ...
        static private void CalcResLegTooWetGround(int startIndex, int endIndex)
        {
            ...
            if (isResLegTooWetGround)
            {
                ...
                UpdateOutputTable(startIndexTemp, startIndexTemp + 23, strResLegTooWetGround, RS_Plant_Maize_Medium_ResLegTooWetGround);
                isResLegTooWetGround = false;
            }
        ...
        static private void CalcResLegRain(int startIndex, int endIndex)
        {
            ...
            {
                strCellValue = "0";//High Risk
                UpdateOutputTable(startIndexTemp, startIndexTemp + 5, strResLegRain, RS_Plant_Maize_Medium_ResLegRain);
                isResLegRain = false;
            }
